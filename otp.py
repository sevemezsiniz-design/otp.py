...
IKINCI_KANAL_LINK = "https://t.me/+BURAYA_IKINCI_KANAL_LINK"  # 2. kaynak kanal (linki güncelle)
...
class MesajUserbot:
    ...
    async def start(self):
        ...
        # Her iki kaynak kanaldan mesajları dinle
        @self.client.on(events.NewMessage(chats=[self.hedef_kanal, self.ikinci_kanal]))
        async def handler(event):
            try:
                # "000-000" formatındaki kodu bul
                pattern = r'\b\d{3}-\d{3}\b'
                kodlar = re.findall(pattern, event.message.text or "")
                kod_bilgisi = f"\n🔢 **Kod:** {kodlar[0]}" if kodlar else ""
                
                # Mesajı kendi kanalına ilet (callback bilgileri dahil)
                mesaj_icerik = f"📨 *Yeni Mesaj İletildi:*\n\n{event.message.text}{kod_bilgisi}"
                
                # Eğer mesajda callback butonu varsa onu da yakala
                if event.message.reply_markup:
                    mesaj_icerik += "\n\n*🔘 Callback Butonu Tespit Edildi*"
                    
                    # Buton metinlerini ekle
                            ...
                            for button in row.buttons:
                                if hasattr(button, 'text'):
                                    ...
                                    if hasattr(button, 'data'):
                                        ...
                                        data_kodlar = re.findall(pattern, button.data.decode() if isinstance(button.data, bytes) else str(button.data))
                                        if data_kodlar:
                                            mesaj_icerik += f" (Data kodu: {data_kodlar[0]})"
                ...
                logger.info(f"Mesaj iletildi: {event.message.text[:50] if event.message.text else 'İçerik yok'}...")
