    ...
    async def start(self):
        ...
        async def handler(event):
            try:
                ...
                logger.info(f"Mesaj alındı: {mesaj_text[:50] if mesaj_text else 'İçerik yok'}")
                                ...
                                for button in row.buttons:
                                    ...
                                    if hasattr(button, 'data'):
                                        ...
                                        if data_kodlar:
                                            ...
                                            buton_bilgileri.append(f"• Buton: '{buton_metni}' → Data kodu: {', '.join(data_kodlar)}")
                                        else:
                ...
                # Mesajdaki kodlar
                if mesaj_kodlar:
                    mesaj_icerik += f"**🔢 MESAJDAKİ KODLAR:**\n{', '.join(mesaj_kodlar)}\n\n"
                
                # Callback butonları
                if buton_bilgileri:
                    mesaj_icerik += f"**🔘 CALLBACK BUTONLARI:**\n"
                    mesaj_icerik += "\n".join(buton_bilgileri) + "\n\n"
                
                # Tüm kodlar özeti
                if tum_kodlar:
                    mesaj_icerik += f"**📌 BULUNAN TÜM KODLAR:**\n{', '.join(tum_kodlar)}"
                else:
                    mesaj_icerik += "**📌 Kod bulunamadı**"
                ...
                if self.hedef_kanalim:
                    ...
                    logger.info(f"Mesaj iletildi - Kodlar: {tum_kodlar}")
