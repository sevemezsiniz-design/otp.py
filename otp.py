    ...
    async def start(self):
        ...
        async def handler(event):
            try:
                ...
                # 2. BUTONLARDAN MESAJ İÇERİĞİ OLUŞTUR
                buton_mesajlari = []
                buton_kodlari = []
                        ...
                        if hasattr(event.message.reply_markup, 'rows'):
                            for row in event.message.reply_markup.rows:
                                for button in row.buttons:
                                    ...
                                    # Buton metnini mesaj içeriği olarak ekle
                                    buton_mesajlari.append(f"🔘 {buton_metni}")
                                    ...
                                    if hasattr(button, 'data'):
                                        ...
                                        if data_kodlar:
                                            ...
                                            buton_mesajlari.append(f"  ↳ Data: {data_str[:50]}")
                    except Exception as e:
                        logger.error(f"Buton işleme hatası: {e}")
                
                # TÜM KODLARI BİRLEŞTİR
                tum_kodlar = list(set(mesaj_kodlar + buton_kodlari))
                
                # MESAJ İÇERİĞİNİ HAZIRLA
                mesaj_icerik = f"📨 *Yeni Mesaj İletildi:*\n\n"
                
                # Orijinal mesaj içeriği (varsa)
                if mesaj_text:
                    mesaj_icerik += f"**📝 MESAJ İÇERİĞİ:**\n{mesaj_text}\n"
                
                # Butonlardan oluşan içerik (callback mesajları için)
                if buton_mesajlari:
                    if mesaj_text:
                        mesaj_icerik += f"\n**🔘 BUTONLAR:**\n"
                    else:
                        mesaj_icerik += f"**📝 MESAJ İÇERİĞİ (Butonlardan):**\n"
                    mesaj_icerik += "\n".join(buton_mesajlari) + "\n"
                ...
                # Eğer hiç içerik yoksa uyarı ver
                if not mesaj_text and not buton_mesajlari and not media_bilgisi:
                    mesaj_icerik += "**📝 MESAJ İÇERİĞİ:**\n*İçerik çözülemedi*"
