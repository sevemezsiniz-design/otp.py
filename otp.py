...
from telethon.tl.types import MessageEntityTextUrl, MessageEntityBold, MessageEntityItalic, MessageMediaPhoto, MessageMediaDocument
import os
import sys
import re
import signal
...
IKINCI_KANAL_LINK = "https://t.me/+6jUMgsaod6QxMDRl"  # 2. kaynak kanal
    ...
    async def start(self):
        ...
        try:
            ...
            if IKINCI_KANAL_LINK:
                ...
                self.kanallar_listesi.append(self.ikinci_kanal)
        ...
        if not self.kanallar_listesi:
            logger.error("Hiçbir kaynak kanala katılamadı!")
            return
        
        # Kaynak kanallardan mesajları dinle
        @self.client.on(events.NewMessage(chats=self.kanallar_listesi))
        async def handler(event):
            try:
                # Mesaj içeriğini al - BOŞSA BİLE AL
                mesaj_text = event.message.text or ""
                logger.info(f"Mesaj alındı - Uzunluk: {len(mesaj_text)}, Buton var mı: {event.message.reply_markup is not None}")
                
                # Mesajda media var mı kontrol et
                media_bilgisi = ""
                if event.message.media:
                    if hasattr(event.message.media, 'photo'):
                        media_bilgisi = "\n🖼️ *Bu mesajda fotoğraf var*"
                    elif hasattr(event.message.media, 'document'):
                        media_bilgisi = "\n📎 *Bu mesajda dosya var*"
                
                # 1. MESAJ İÇİNDEKİ KODLARI BUL (000-000 formatı)
                pattern = r'\b\d{3}-\d{3}\b'
                mesaj_kodlar = re.findall(pattern, mesaj_text)
                
                # 2. CALLBACK BUTONLARINI VE KODLARINI BUL
                buton_bilgileri = []
                buton_kodlari = []
                
                if event.message.reply_markup:
                    try:
                        # Telethon'un reply_markup yapısını kontrol et
                        if hasattr(event.message.reply_markup, 'rows'):
                            for row in event.message.reply_markup.rows:
                                for button in row.buttons:
                                    buton_metni = getattr(button, 'text', 'Metin yok')
                                    buton_bilgileri.append(f"• Buton: '{buton_metni}'")
                                    
                                    # Buton metnindeki kodları bul
                                    buton_metin_kodlar = re.findall(pattern, buton_metni)
                                    if buton_metin_kodlar:
                                        buton_kodlari.extend(buton_metin_kodlar)
                                        buton_bilgileri.append(f"  ↳ Butonda kod: {', '.join(buton_metin_kodlar)}")
                                    
                                    # Callback data'daki kodları bul
                                    if hasattr(button, 'data'):
                                        data_str = button.data.decode('utf-8', errors='ignore') if isinstance(button.data, bytes) else str(button.data)
                                        data_kodlar = re.findall(pattern, data_str)
                                        if data_kodlar:
                                            buton_kodlari.extend(data_kodlar)
                                            buton_bilgileri.append(f"  ↳ Data'da kod: {', '.join(data_kodlar)}")
                    except Exception as e:
                        logger.error(f"Buton işleme hatası: {e}")
                        buton_bilgileri.append(f"• Buton işlenirken hata: {str(e)[:50]}")
                
                # TÜM KODLARI BİRLEŞTİR
                tum_kodlar = list(set(mesaj_kodlar + buton_kodlari))  # Benzersiz kodlar
                
                # MESAJI OLUŞTUR - KESİNLİKLE MESAJ İÇERİĞİNİ GÖSTER
                mesaj_icerik = f"📨 *Yeni Mesaj İletildi:*\n\n"
                
                # Orijinal mesaj - BOŞ BİLE OLSA GÖSTER
                if mesaj_text:
                    mesaj_icerik += f"**📝 MESAJ İÇERİĞİ:**\n{mesaj_text}\n"
                else:
                    mesaj_icerik += f"**📝 MESAJ İÇERİĞİ:**\n*Metin içeriği yok*\n"
                
                # Media bilgisi varsa ekle
                mesaj_icerik += media_bilgisi
                ...
                if buton_bilgileri:
                    mesaj_icerik += f"\n\n**🔘 CALLBACK BUTONLARI:**\n"
                    mesaj_icerik += "\n".join(buton_bilgileri)
                
                # Tüm kodlar özeti - her zaman göster
                if tum_kodlar:
                    mesaj_icerik += f"\n\n**📌 BULUNAN TÜM KODLAR:**\n{', '.join(tum_kodlar)}"
                else:
                    # Kod bulunamadı mesajını kaldırdım, çünkü resimdeki kod zaten mesaj içeriğinde görünecek
                    pass
                ...
                if self.hedef_kanalim:
                    ...
                    logger.info(f"Mesaj iletildi - Uzunluk: {len(mesaj_icerik)}")
